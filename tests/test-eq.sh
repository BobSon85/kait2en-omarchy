...
[[ -f "$CONF" ]]
[[ $(grep -cE 'label = bq_peaking' "$CONF") -eq 8 ]]
grep -qE 'filter.smart.name = "filter.sink.kait2en-user-eq"' "$CONF"
grep -qE 'filter.smart.target = \{ node.name = "audio_effect.t2-81-speakers" \}' "$CONF"
